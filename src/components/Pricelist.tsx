...
export function Pricelist() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const allData = XLSX.utils.sheet_to_json<Row>(ws, { header: 1, defval: "" }); //defval ="" to avoid 'undefined' cells when empty
    ...
    return (
        <section id="pricelist" className="relative pt-8 w-full gap-y-8 min-h-screen flex flex-col items-center bg-primary/7">
            <Label>Pilih Gender:</Label>
            <Select onValueChange={(val) => setGender(val)}> {/* val si always a string that dont return value*/}
            ...
            <Slider defaultValue={[age]} max={70} step={1} onValueChange={(v) => setAge(v[0])} className="w-1/4"/>
            <span>{age}</span>
            <Table className="w-4/5 border-separate border-4 text-center mx-auto">
                ...
                <TableHeader className="bg-blue-200">
                    <TableRow className="hover:bg-transparent">
                        <TableHead className="text-center" rowSpan={2}>Usia (tahun)</TableHead>
                        <TableHead className="text-center" rowSpan={2}>Harga/tahun</TableHead>
                        <TableHead className="text-center" colSpan={3}>Wilayah Cover</TableHead>
                        <TableHead className="text-center" colSpan={2}>Tipe Kamar</TableHead>
                        <TableHead className="text-center" rowSpan={2}>Limit per tahun</TableHead>
                        <TableHead className="text-center" rowSpan={2}>Booster</TableHead>
                    </TableRow>
                    <TableRow className="hover:bg-transparent">
                        <TableHead className="text-center">Indonesia</TableHead>
                        <TableHead className="text-center">Asia (kecuali SG, HK, JP)</TableHead>
